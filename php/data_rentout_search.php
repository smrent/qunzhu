...
if($rent_type_get == ""){//按关键字搜索，已禁止空关键字搜索
	...
	if ($num_content > 0){
		...
		$sql="select * from house_rent where (content like '%$keyword_content%' or title like '%$keyword_content%') and is_show = 1 and verify =1  order by (set_top = 1) desc, addtime desc limit $last,$amount";// 
  ...
  if($count==0){//5个条件都是不限
		$sql="select * from house_rent where is_show = 1 and verify =1 order by (set_top = 1) desc, addtime desc limit $last,$amount";
	}
	if($count==1){//1个条件不是不限
		$sql="select * from house_rent where $a0 = '$b0' and is_show = 1 and verify =1 order by (set_top = 1) desc, addtime desc limit $last,$amount";
	}
	if($count==2){//2个条件不是不限
		$sql="select * from house_rent where $a0 = '$b0' and $a1 = '$b1' and is_show = 1 and verify =1 order by (set_top = 1) desc, addtime desc limit $last,$amount";
		//echo $sql;
	}
	if($count==3){//3个条件不是不限
		$sql="select * from house_rent where $a0 = '$b0' and $a1 = '$b1' and $a2 = '$b2' and is_show = 1 and verify =1 order by (set_top = 1) desc, addtime desc limit $last,$amount";
		//echo $sql;
	}
...
//	$star_sum=$row2['star_sum'];
	...
	if($set_top== 1){//如果置顶
		...
		$content=mb_substr($content,0,50,'utf-8');//有标题，搜索的正文最多只能容纳74个字
	}else{//如果未置顶
		//$title=mb_substr($content,0,17,'utf-8');
		//$title="";//未置顶 没有特写标题 则标题不显示
		$content=mb_substr($content,0,50,'utf-8');//标题为空，搜索的正文容纳100个字
	}
...
//	}
	...
	//$time=time();
	$district=$row['district']."/".$row['rent_type']."/".$row['room_type'];
	$time0=$row['addtime'];
