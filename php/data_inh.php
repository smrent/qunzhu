...
$sql="select * from house_rent where is_show = 1 and verify =0 order by (set_top = 1) desc, addtime desc limit $last,$amount";
$result=$mysqli->query($sql);
while ($row=$result->fetch_array(MYSQLI_ASSOC)) {
	$house_seek_id=$row['id'];
	$bianhao="W".$house_seek_id;
	$contact=$row['contact'];
	$rs1=$mysqli->query("select jifen from `user` WHERE id = '$contact' and is_show = 1");
	...
	$sayList[] = array(
		...
		//'headimg'=>$row['headimg'],
		'pic-wrapper-float'=>$row['cover_img'],
		'district'=>$district,
		//'room_type'=>$row['room_type'],
		//'rent_type'=>$row['rent_type'],
		//'view'=>$row['view'],
